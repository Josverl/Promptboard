...
class Pushbutton:
    ...
    # ****** API ******
    def press_func(self, func:Callable|bool|None=False, args=()):
        if func is None:
            self.press = asyncio.Event()
        self._tf = self.press.set if func is None else func
        self._ta = args

    def release_func(self, func:Callable|bool|None=False, args=()):
        if func is None:
            self.release = asyncio.Event()
        self._ff = self.release.set if func is None else func
        self._fa = args

    def double_func(self, func:Callable|bool|None=False, args=()):
        if func is None:
    ...
    def long_func(self, func:Callable|bool|None|None=False, args=()):
        if func is None:
